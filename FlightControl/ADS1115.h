...
class ADS1115 {
    public:
		...
		void setComparatorHiThresh(uint16_t hiThreshInMilliVolts,uint16_t gainInVolts);
		void setComparatorLoThresh(uint16_t loThreshInMilliVolts,uint16_t gainInVolts);


    private:
        void updateConfigRegister();
		void updateHiThreshRegister(uint16_t value);
		void updateLoThreshRegister(uint16_t value);
