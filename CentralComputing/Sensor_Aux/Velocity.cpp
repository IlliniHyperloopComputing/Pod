...
Arbitrary_Data velocity_calculation(Arbitrary_Data raw) {
  ...
  double final = rpm / SECONDS * 2 * M_PI * ENCODER_RADIUS;					//final velocity calculation --> rpm into seconds				
  Velocity_Calc * v_calc = (Velocity_Calc *) malloc(sizeof(Velocity_Calc)); //space for the calculated veloctiy-> 8 bits
  v_calc->velocity = final;                                                  
...
void velocity_parse(uint8_t * buffer, Arbitrary_Data raw) {
  ...
  v_raw->tape_count = (int) *(buffer + TAPE_OFFSET);                         // Fetch the tape count from the buffer and store
  v_raw->encoder_count = (int) *(buffer  + ENCODER_COUNT_OFFSET);            // Fetch the encoder count from the buffer and store 
}
