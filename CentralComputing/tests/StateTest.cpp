...
TEST_F(PodTest, ConnectDisconnect) {
	...
	MoveState(NetworkManager::Network_Command_ID::TRANS_CONNECT_DISCONNECT,Pod_State::E_States::ST_CONNECT_DISCONNECT,true);
...
TEST_F(PodTest, FunctionalTest) {
  	...
  	MoveState(NetworkManager:Network_Command_ID::TRANS_CONNECT_DISCONNECT, Pod_State::E_States::ST_CONNECT_DISCONNECT,true);//this transition should fail because we cannot skip loading
  	MoveState(NetworkManager::Network_Command_ID::TRANS_SAFE_MODE, Pod_State::E_States::ST_SAFE_MODE, true);
}
...
TEST_F(PodTest,LaunchReady){
	MoveState(NetworkManager:Network_Command_ID::TRANS_LAUNCH_READY, Pod_State::E_States::ST_LAUNCHREADY,true);
	MoveState(NetworkManager:Network_Command_ID::TRANS_CONNECT_DISCONNECT, Pod_State::E_States::ST_CONNECT_DISCONNECT,true);
	MoveState(NetworkManager::Network_Command_ID::TRANS_FUNCTIONAL_TEST, Pod_State::E_States::ST_FUNCTIONAL_TEST, true);
	MoveState(NetworkManager::Network_Command_ID::TRANS_SAFE_MODE, Pod_State::E_States::ST_SAFE_MODE, true);
}
