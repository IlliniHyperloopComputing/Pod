...
def bytes_to_uint(bytes, length):
        new_bytes = []
...
def bytes_to_uint16(bytes, length):
        new_bytes = []
        for x in range(0, length):
                new_bytes.append(int.from_bytes(
                bytes[(x*2):(x*2+2)], byteorder='little', signed=False))
        return new_bytes
...
def bytes_to_int8(bytes, length):
        new_bytes = []
        for x in range(0, length):
                new_bytes.append(int.from_bytes(
                bytes[(x):(x+1)], byteorder='little', signed=True))
        return new_bytes
